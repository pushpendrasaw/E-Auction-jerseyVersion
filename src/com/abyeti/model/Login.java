...
public class Login {
	...
	public Response isLoggedIn() throws Exception {
		...
		jsonObject.put("MSG",
				"Welcome,  " + Functions.getLoggedInUsername(request));
		returnString = jsonArray.put(jsonObject).toString();
		return Response.ok(returnString).build();
	}
	...
	public Response LoginUser(String incomingData) throws Exception {
		...
		try {
			...
			if (rs.next()) {
				...
				session.setAttribute("eauction_username",
						userData.optString("username"));
				System.out.println("eauction_sellerbuyer"+rs.getString("sellerbuyer"));
				session.setAttribute("eauction_sellerbuyer",rs.getString("sellerbuyer"));
				returnString = jsonArray.put(jsonObject).toString();
				...
				jsonObject.put("MSG",
						"<p class='text-danger'>Login Unsuccessful</p>");
				returnString = jsonArray.put(jsonObject).toString();
			}
	...
	public Response addUser(String incomingData) throws Exception {
		...
		try {
			...
			ps.setString(1, userData.optString("username"));
			ps.setString(2, userData.optString("password"));
			ps.setString(3, userData.optString("firstname"));
			ps.setString(4, userData.optString("lastname"));
			ps.setString(5, userData.optString("emailid"));
			ps.setString(6, userData.optString("sellerbuyer"));
			...
			if (http_code != 0) {
				...
				return Response.status(500).entity("Unable to add user")
						.build();
			}
